...
export function BlogCard({ blog }) {
  return (
    <Link href={`/dashboard/blogs/${blog.id}`}>
      <a key={blog.id}>
        <div className="p-6 bg-white rounded-lg border border-gray-200 select-none">
          ...
          <p className="mb-8 font-light text-gray-500">{blog.deskripsi}</p>
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              <img className="w-7 h-7 rounded-full" src="https://flowbite.s3.amazonaws.com/blocks/marketing-ui/avatars/jese-leos.png" alt="Jese Leos avatar" />
              <span className="font-medium dark:text-white">Jese Leos</span>
            </div>
            {/* <Link href={/`blog.judul`}> */}
            <a className="inline-flex items-center font-medium text-sky-600">
              Read more
